...
pub fn test() {
    ...
    text_state.text_area = Rect::from((
        (0.0, 0.0),
        (200.0, 25.0)
    ));
    text_state.recalculate(&mut ctx, true, UpdateReason::Unknown);
    let mono_width = ctx.text_manager.get_position_of_last_glyph(&text_id).unwrap().width;
    ...
    assert_eq!(text_state.relative_caret_offset_horizontal, mono_width * 3.0);

    println!("{:?}: {}", text_state.relative_caret_offset_horizontal, text_state.cursor_before_glyph);
    ...
    assert_eq!(text_state.relative_caret_offset_horizontal, mono_width * 4.0);
    ...
    assert_eq!(text_state.relative_caret_offset_horizontal, mono_width * 5.0);
}
